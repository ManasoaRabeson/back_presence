...
@section('content')
    <div class="w-full h-full container mt-4">
        @if (count($formateurs) <= 0)
            <div>
                Aucun formateur de centre de formation professionnelle
            </div>
        @else
            {{-- Début tableau --}}
            <div class="w-full p-4 rounded-3xl bg-white">
                ...
                <div class="mb-4">
                    <input type="text" id="searchInput" placeholder="Rechercher un formateur ..." class="w-full p-2 border rounded">
                </div>
                ...
                <table class="table table-hover caption-top">
                    <caption class="text-2xl font-medium text-slate-800">
                        Liste des formateurs de centre de formation professionnelle
                    </caption>
                    <thead>
                        <tr>
                    ...
                    <tbody id="moduleTableBody">
                        @foreach ($formateurs as $formateur)
                            <tr>
                                ...
                                <td>
                                    <div>
                                        {{ $formateur->customerName }}
                                    </div>
                                </td>
                                <td>
                                    <div>
                                        <a href="/superAdmins/formateurs/cfp/delete/{{ $formateur->idFormateur }}"><i class="fa-solid fa-trash-can" title="Supprimer définitivement"></i></a>
                                    </div>
                                </td>
                            </tr>
                        @endforeach
