<div class="grid w-full min-w-[550px] grid-cols-12 gap-x-6 p-2 border-[1px] border-gray-200 rounded-xl">
    ...
    <div class="grid w-full min-[350px]:col-span-12 md:col-span-2 grid-cols-subgrid">
        ...
        <div class="grid w-full grid-cols-12 md:hidden gap-x-4">
            ...
            {{-- Module + ETP || CFP + Sessions + Heures + Apprenants --}}
            <div class="grid w-full p-2 h-full min-[350px]:col-span-6 grid-cols-subgrid">
                <div class="grid w-full grid-cols-6">
                    ...
                    <div class="grid col-span-6 grid-cols-subgrid">
                        ...
                        @if (isset($p['idCfp']))
                            @if (isset($p['logoCfp']))
                                <div onclick="showCustomer({{ $p['idCfp'] }}, '/etp/cfp-drawer/')"
                                    class="relative h-16 bg-gray-200 cursor-pointer w-28 rounded-xl">
                                    <x-icon-badge />
                                    <img src="/img/entreprises/{{ $p['logoCfp'] }}" alt="logo"
                                        class="object-cover w-full h-full rounded-xl">
                                </div>
                            @elseif (!isset($p['logoCfp']) && isset($p['initialnameCfp']))
                                <span onclick="showCustomer({{ $p['idCfp'] }}, '/etp/cfp-drawer/')"
                                    class="cursor-pointer"
                                    style="background: #e5e7eb; padding: 2px 8px; border-radius: 50%; cursor: pointer;">{{ $p['initialnameCfp'] }}</span>
                            @else
                                @if (isset($p['etp_name']))
                                    <div onclick="showCustomer({{ $p['etp_logo'] }}, '/etp/cfp-drawer/')"
                                        class="relative h-16 bg-gray-200 cursor-pointer w-28 rounded-xl">
                                        <x-icon-badge />
                                        <img src="/img/entreprises/{{ $p['etp_logo'] }}" alt="logo"
                                            class="object-cover w-full h-full rounded-xl">
                                    </div>
                                    <span onclick="showCustomer({{ $p['etp_logo'] }}, '/etp/cfp-drawer/')"
                                        class="text-gray-400 capitalize">{{ $p['etp_name'] }}
                                    </span>
                                @else
                    ...
                    <div class="btn-group h-max">
                        ...
                        <ul class="dropdown-menu">
                            <x-dropdown-item icontype="solid" icon="eye"
                                route="{{ route('etp.projets.show', $p['idProjet']) }}" label="Aperçu" />

                            @if ($p['project_status'] != 'Terminé' && $p['project_type'] == 'Interne')
                                {{-- {{ dd($p) }} --}}
                                ...
                                <x-dropdown-item icontype="solid" icon="trash" type="btn"
                                    onclick="showModalConfirmation({{ $p['idProjet'] }}, 'Supprimer')"
                                    label="Supprimer" />
                                <x-dropdown-item icontype="solid" icon="copy" type="btn"
                                    onclick="showModalConfirmation({{ $p['idProjet'] }}, 'Dupliquer')"
                                    label="Dupliquer" />

                                <hr class="border-[1px] border-gray-400 my-2">
                                @if ($p['project_status'] != 'Annulé')
                                    <x-dropdown-item icontype="solid" icon="xmark" type="btn"
                                        onclick="showModalConfirmation({{ $p['idProjet'] }}, 'Annuler')"
                                        label="Annuler" />
                                @endif
            ...
            <div class="grid w-full col-span-12 grid-cols-subgrid">
                ...
                <div class="grid w-full grid-cols-3">
                    ...
                    <div class="inline-flex items-center w-full">
                        ...
                        {{-- @if (isset($p['total_ttc']))
                  <p class="font-medium text-gray-500">Ar
                      {{ number_format($p['total_ttc'], 0, '.', '.') }} TTC
                  </p>
              @else
                  <p class="font-medium text-gray-500">Ar -- TTC</p>
              @endif --}}
                    </div>
                </div>
            </div>
        ...
        <div class="min-[350px]:hidden md:grid grid-cols-10 w-full">
            <div class="grid w-full md:col-span-4 lg:col-span-2 grid-cols-subgrid">
                <div class="grid w-full col-span-4 grid-cols-subgrid">
                    <h1 onclick="showFormation({{ $p['idModule'] }})" class="text-xl font-medium text-gray-600"
                        title="{{ $p['module_name'] }}">
                        @if (isset($p['module_name']) && $p['module_name'] != 'Default module')
                            {{ $p['module_name'] }}
                        @else
                    ...
                    <span class="inline-flex items-center gap-x-3">
                        ...
                        <div class="grid col-span-6 grid-cols-subgrid">
                            ...
                            @if (isset($p['idCfp']))
                                ...
                                @else
                                    @if (isset($p['etp_name']))
                                        <div onclick="showCustomer({{ $p['etp_logo'] }}, '/etp/etp-drawer/')"
                                            class="relative h-16 bg-gray-200 cursor-pointer w-28 rounded-xl">
                                            <x-icon-badge />
                                            <img src="/img/entreprises/{{ $p['etp_logo'] }}" alt="logo"
                                                class="object-cover w-full h-full rounded-xl">
                                        </div>
                                        <span onclick="showCustomer({{ $p['etp_logo'] }}, '/etp/etp-drawer/')"
                                            class="text-gray-400 capitalize">{{ $p['etp_name'] }}
                                        </span>
                                    @else
                                        <span class="text-gray-400">--</span>
                            ...
                            @else
                                @if (isset($p['etp_logo']))
                                    <div class="relative h-16 bg-gray-200 w-28 rounded-xl">
                                        <x-icon-badge />
                                        <img src="/img/entreprises/{{ $p['etp_logo'] }}" alt="logo"
                                            class="object-cover w-full h-full rounded-xl">
                                    </div>
                        ...
                        <ul class="dropdown-menu">
                            ...
                            @if ($p['project_status'] != 'Terminé' && $p['project_type'] == 'Interne')
                                ...
                                @if ($p['project_status'] == 'Terminé')
                                    <x-dropdown-item icontype="solid" icon="right-left" type="btn"
                                        onclick="showModalConfirmation({{ $p['idProjet'] }}, 'Cloturer')"
                                        label="Cloturer" />
                                @endif
