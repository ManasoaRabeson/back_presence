                            ...
                            @if (count($p['etp_name']) > 0 && count($p['etp_name']) <= 3)
                                @foreach ($p['etp_name'] as $etp)
                                    @if (isset($etp->etp_logo))
                                        <div onclick="showCustomer({{ $etp->idEtp }}, '/cfp/etp-drawer/')"
                                    ...
                                    @elseif (!isset($etp->etp_logo) && isset($etp->etp_name))
                                        <span onclick="showCustomer({{ $etp->idEtp }}, '/cfp/etp-drawer/')"
                                            title="{{ $etp->etp_name }}"
                        ...
                        <div class="inline-flex items-center gap-4">
                            <div class="">
                                <p class="text-gray-600">{{ $p['seanceCount'] }} <span
                                        onclick="showSessions('/cfp/session-drawer/{{ $p['idProjet'] }}')"
                ...
                <div class="grid items-start justify-end h-full col-span-1">
                    <div class="btn-group h-max">
                        ...
                        <ul class="shadow-md dropdown-menu w-[220px] rounded-xl border-[1px] border-gray-200">
                            ...
                            @endif
                            @if ($p['project_type'] == 'Inter')
                                @if ($p['project_inter_privacy'] == 0)
                                    <x-dropdown-item icontype="solid" icon="lock-open" type="btn"
                                        onclick="showModalConfirmation({{ $p['idProjet'] }}, 'RendrePublic')"
                                        label="Rendre publique" />
                                @elseif ($p['project_inter_privacy'] == 1)
                                    <x-dropdown-item icontype="solid" icon="lock" type="btn"
                                        onclick="showModalConfirmation({{ $p['idProjet'] }}, 'RendrePrivee')"
                                        label="Rendre privé" />
                                @endif
                            @endif
            ...
            <div class="grid w-full col-span-12 grid-cols-subgrid">
                ...
                <div class="grid w-full grid-cols-3">
                    ...
                    <div class="inline-flex items-center w-full">
                        ...
                        <p onclick="showDocuments('/cfp/document-drawer/{{ $p['idProjet'] }}')"
                            class="font-medium cursor-pointer text-gray-500">{{ $p['nbDocument'] }} Doc</p>
