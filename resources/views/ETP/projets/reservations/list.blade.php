                ...
                <div class="grid w-full grid-cols-6">
                    ...
                    <div class="grid col-span-6 grid-cols-subgrid">
                        <span class="inline-flex items-center gap-x-3">
                            @if (isset($cfp->logoCfp))
                                <div onclick="showCustomerCfp({{ $cfp->idCustomer }}, '/etp/etp-drawer/')"
                                    class="relative w-20 h-10 capitalize bg-gray-200 cursor-pointer rounded-xl"
                                    title="{{ $cfp->nameCfp }}">
                                    <x-icon-badge />
                                    <img src="/img/formateurs/{{ $cfp->logoCfp }}" alt="logo"
                        ...
                        <div class="inline-flex items-center gap-4">
                            ...
                            <div class="">
                                <p class="text-gray-600">{{ $p['apprCount'] }} <a href="#"><span
                                            onclick="showApprenants('/cfp/apprenant-drawer/{{ $p['idProjet'] }}')"
                    ...
                    <div class="btn-group h-max">
                        ...
                        <ul class="dropdown-menu">
                            ...
                            <x-dropdown-item icontype="solid" icon="check" type="btn"
                                onclick="showModalConfirmation({{ $p['idProjet'] }}, 'Valider')"
                                label="Valider le projet" />
                            <x-dropdown-item icontype="solid" icon="trash" type="btn"
                                onclick="showModalConfirmation({{ $p['idProjet'] }}, 'Supprimer')" label="Supprimer" />
                            <x-dropdown-item icontype="solid" icon="copy" type="btn"
                                onclick="showModalConfirmation({{ $p['idProjet'] }}, 'Dupliquer')" label="Dupliquer" />

                            <hr class="border-[1px] border-gray-400 my-2">
                            @if ($p['project_status'] != 'Annulé')
                                <x-dropdown-item icontype="solid" icon="xmark" type="btn"
                                    onclick="showModalConfirmation({{ $p['idProjet'] }}, 'Annuler')" label="Annuler" />
                            @endif
                            @if ($p['project_status'] != 'Reporté')
                                <x-dropdown-item icontype="solid" icon="right-left" type="btn"
                                    onclick="showModalConfirmation({{ $p['idProjet'] }}, 'Reporter')"
                                    label="Reporter" />
                            @endif
                            {{-- @endif --}}
                        </ul>
                ...
                <div class="grid items-end justify-end col-span-1">
                    <div class="inline-flex items-center justify-end gap-1">
                        ...
                        <p class="font-medium text-gray-500 p_note_{{ $p['idProjet'] }}">
                            {{ $p['general_note'] ? number_format($p['general_note'], 1, ',', ' ') : '0' }}</p>
                        <span class="text-gray-400">
                            ({{ $p['general_note'] ? $p['general_note'] : '0' }} avis)
                        </span>
                        <div id="raty_notation_lg_{{ $p['idProjet'] }}"
                            data-val="{{ $p['general_note'] ? $p['general_note'] : '0' }}"
    ...
    <div class="grid w-full h-full min-[350px]:col-span-12 md:col-span-10 grid-cols-subgrid">
        <div class="min-[350px]:grid w-full grid-cols-12 md:hidden">
                    ...
                    <div class="grid justify-end col-span-1">
                        <div class="inline-flex items-center gap-x-2">
                            <i title="Petit déjeuner" data-bs-toggle="tooltip"
                                class="text-lg text-gray-200 cursor-pointer fa-solid fa-bread-slice"></i>
                            <i title="Pause café" data-bs-toggle="tooltip"
                                class="text-lg text-gray-200 cursor-pointer fa-solid fa-mug-hot"></i>
                            <i title="Déjeuner" data-bs-toggle="tooltip"
                                class="text-lg text-gray-200 cursor-pointer fa-solid fa-bowl-rice"></i>
                            <i title="Pause café" data-bs-toggle="tooltip"
                                class="text-lg text-gray-600 cursor-pointer fa-solid fa-mug-hot"></i>
                            <i title="Dîner" data-bs-toggle="tooltip"
                                class="text-lg text-gray-200 cursor-pointer fa-solid fa-utensils"></i>
                            <i title="Bouteille d'eau" data-bs-toggle="tooltip"
                                class="text-lg text-gray-600 cursor-pointer fa-solid fa-bottle-water"></i>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="grid w-full col-span-12 grid-cols-subgrid">
                <hr class="border-[1px] w-full border-gray-200 my-2">
                <div class="grid w-full grid-cols-3">
                    <div class="grid w-full col-span-1">
                        <div class="inline-flex items-center w-full">
                            <div class="w-[24px] flex justify-center items-center">
                                <i class="text-gray-400 fa-solid fa-sack-dollar"></i>
                            </div>
                            <p class="font-medium text-gray-500">
                                @if ($p['paiement'] != 'Autres')
                                    {{ $p['paiement'] }}
                                @else
                                    --
                                @endif
                    ...
                    <div class="grid w-full col-span-1">
                        <div class="inline-flex items-center w-full">
                            <div class="w-[24px] flex justify-center items-center">
                                <i class="text-gray-400 fa-solid fa-dollar"></i>
                            </div>
                            @if (isset($p['projectTotalPrice']))
                                <p class="font-medium text-gray-500">Ar
                                    {{ number_format($p['projectTotalPrice'], 0, '.', '.') }} HT</p>
                            @else
                                <p class="font-medium text-gray-500">Ar -- HT</p>
                            @endif
                        </div>
                    </div>
                    <div class="inline-flex items-center w-full">
                        ...
                        <p class="font-medium text-gray-500">8 Documents</p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="min-[350px]:hidden md:grid grid-cols-10 w-full">
            <div class="grid w-full md:col-span-4 lg:col-span-2 grid-cols-subgrid">
                ...
                <div class="grid w-full col-span-4 grid-cols-subgrid">
                    <span class="inline-flex items-center gap-x-3">
                        <div onclick="showCustomerCfp({{ $p['idCfp_inter'] }}, '/etp/etp-drawer/')"
                            class="relative w-20 h-10 capitalize bg-gray-200 cursor-pointer rounded-xl"
                            title="{{ $p['nameCfp'] }}">
                            <x-icon-badge />
                            @if (isset($p['logoCfp']))
                                <img src="/img/entreprises/{{ $p['logoCfp'] }}" alt="logo"
                                    class="object-cover w-full h-full rounded-xl">
                            @else
                                <span class="flex items-center justify-center mt-2">{{ $p['nameCfp'][0] }}</span>
                            @endif
                        </div>
                    </span>
                </div>
                ...
                <div class="w-full col-span-4 md:grid lg:hidden grid-cols-subgrid">
                    <div class="inline-flex items-center w-full gap-4">
                        ...
                        <div class="">
                            <p class="text-gray-600">{{ $p['apprCount'] }} <a href="#"><span
                                        onclick="showApprenants('/etp/apprenant-drawer/{{ $p['idProjet'] }}')"
                                        class="text-gray-400 underline cursor-pointer">Apprenants</span></a></p>
                        </div>
                    </div>
                </div>
            ...
            <div class="w-full lg:grid min-[350px]:hidden lg:col-span-2 gap-y-0 grid-cols-subgrid">
            ...
            <div class="grid w-full md:col-span-4 lg:col-span-4 gap-y-0 grid-cols-subgrid">
                    ...
                    <div class="grid w-full grid-cols-2">
                        <div class="grid w-full gap-2 md:col-span-2 lg:col-span-1 grid-cols-subgrid">
                            <div class="flex flex-col w-full">
                                <div class="flex w-full lg:flex-col md:flex-row lg:items-start md:items-center">
                                    ...
                                    <span data-bs-toggle="tooltip"
                                        class="inline-flex items-center gap-2 px-2 py-1 text-sm w-[90px] justify-center"
                            ...
                            <div class="w-full">
                                <div class="flex w-full lg:flex-col md:flex-row lg:items-start md:items-center">
                                    ...
                                    <div class="md:hidden lg:block w-[24px] flex flex-col">
                                        <p class="text-gray-400">Reservation</p>
                                    </div>
                                    <p class="flex flex-row items-center gap-2 text-gray-600">
                                        <i class="text-gray-400 fa-solid fa-thumbtack"></i> {{ $p['nbPlace'] }} places
                                        reservés
                                    </p>
                                </div>
                            </div>
                        ...
                        <div class="grid w-full gap-2 md:col-span-2 lg:col-span-1 grid-cols-subgrid">
                            <div class="flex justify-between w-full">
                                ...
                                @if ($p['is_active_inter'] == 1)
                                    <div>
                                        <a onclick="__global_drawer('offcanvasApprenant', this)"
                                            data-id="{{ $p['idProjet'] }}" data-nb_place="{{ $p['nbPlace'] }}"
                                            ...
                                            <span data-bs-toggle="tooltip" title="Ajouter vos apprenants"><i class="text-blue-500 fa-solid fa-user-plus fa-lg"></i>
                                        </a>
                                    </div>
                                @endif
                            </div>
                            ...
                            @else
                                <div class="inline-flex items-center w-full space-x-2">
                                    <p>Vous ne pouvez pas ajouter vos apprenants tant que votre réservation n'est pas validée.</p>
                                </div>
                            @endif
            ...
            <div class="grid w-full md:col-span-2 lg:col-span-2 grid-cols-subgrid">
                <div class="flex justify-end w-full h-full gap-1">
                    <span
                        class="inline-flex  gap-2 w-[90px] justify-end
                                        @switch($p['modalite'])
                                          @case('Présentielle')
                                            text-[#00b4d8]
                                            @break
