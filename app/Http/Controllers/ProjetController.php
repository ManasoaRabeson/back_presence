...
class ProjetController extends Controller
{
    ...
    public function getProjectList()
    {
        ...
        $projectDates = $this->project->headDate(Customer::idCustomer())
            ->where('project_status', 'En cours')
            ->groupBy('headDate')
            ->orderBy('dateDebut', 'asc')
            ->get();

        $projectCount = count($this->project->index(Customer::idCustomer())->get());
    ...
    public function getCountProject()
    {
        ...
        $projetTermines = $this->project->countByStatus(Customer::idCustomer(), "Terminé");
        $projetClotures = $this->project->countByStatus(Customer::idCustomer(), "Cloturé");
    ...
    public function getNomDossier($idProjet, $dossiers)
    {
        $val = [];

        foreach ($dossiers as $d) {
            if ($d->idProjet == $idProjet) {
                $val[] = $d;
            }
        }

        return count($val) > 0 ? $val : null;
    }
    ...
    {
        Log::info('Requete dans taxe : ' . $idProjet);
        $taxe = $request->input('taxe');
        DB::table('projets')
            ->where('idProjet', $idProjet)
            ->update(['taxe' => $taxe]);

        return response()->json(['success' => 'Taxe mise à jour avec succès.']);
    }
    ...
    public function fraisTotal($idProjet)
    {
        try {
            ...
            $fraisTotal = DB::table('fraisprojet')
                ...
                ->first();
            if ($fraisTotal === null) {
                return response()->json(['error' => 'Aucun frais trouvé pour ce projet'], 404);
            }
    ...
    public function getTotalHTJSON($idProjet)
    {
        try {
            ...
            $fraisTotalHT = $TotalHT->total_ht;
            return response()->json(['total_ht' => $fraisTotalHT]);
        } catch (Exception $e) {
            return response()->json(['error' => 'Une erreur est survenue', 'details' => $e->getMessage()]);
        }
    }
    ...
    public function getTotalTTC($idProjet)
    {
        try {
            $TotalTTC = DB::table('projets')
                ->select('total_ttc')
                ->where('idProjet', $idProjet)
                ->first();

            $fraisTotalTTC = $TotalTTC->total_ttc;
    ...
    public function updateFrais(Request $request)
    {
        ...
        $validator = Validator::make($request->all(), [
            ...
            'idFraisProjet' => 'required|exists:fraisprojet,idFraisProjet' // Assurez-vous que idFraisProjet existe dans la table fraisprojet
        ]);
        ...
        try {
            ...
            $idFraisProjet = $request->input('idFraisProjet');
            $montant = $request->input('montant');
            $description = $request->input('description');

            // Mettre à jour la table fraisprojet
            DB::table('fraisprojet')
                ->where('idFraisProjet', $idFraisProjet)
                ->update([
                    'montant' => $montant,
            ...
            // Récupérer l'ID du projet associé
            $idProjet = ProjetController::getIdProjetByIdFraisProjet($idFraisProjet);

            // Mettre à jour les totaux des frais
    ...
    public function fraisAssign($idProjet, $idFrais, $isEtp)
    {
        ...
        try {
            ...
            $insert = DB::table('fraisprojet')->insert([
                'idProjet' => $idProjet,
                'idFrais' => $idFrais,
                'montant' => 0,
                'description' => null,
            ...
            if ($insert) {
                ...
                DB::rollBack();
                Log::error('Erreur lors de l\'insertion des données', ['idProjet' => $idProjet, 'idFrais' => $idFrais]);
                return response()->json(['error' => 'Erreur lors de l\'insertion des données'], 500);
            }
        } catch (\Exception $e) {
            DB::rollBack();
            Log::error('Erreur serveur : ' . $e->getMessage(), ['idProjet' => $idProjet, 'idFrais' => $idFrais]);
    ...
    {
        try {
            $delete = DB::table('fraisprojet')->where('idFraisProjet', $idFraisProjet)->where('idProjet', $idProjet)->delete();
    ...
    public function fermeturefrais()
    {
        try {
            DB::table('fraisprojet')->where('montant', 0)->delete();
    ...
    public function filterItems(Request $req)
    {
        ...
        if ($idFormateurs[0] != null) {
            ...
            $etps = DB::table('v_union_projets')
                ...
                ->leftJoin('project_forms', 'project_forms.idProjet', '=', 'v_union_projets.idProjet')
                ->leftJoin('forms', 'forms.idFormateur', '=', 'project_forms.idFormateur')
                ->leftJoin('customers', 'entreprises.idCustomer', '=', 'customers.idCustomer')
                ->where(function ($query) {
                    $query->where('idCfp_inter', Customer::idCustomer())
                        ->orWhere('idCfp_intra', Customer::idCustomer());
                })
                ->whereIn('project_forms.idFormateur', $idFormateurs)
                ->groupBy(DB::raw('COALESCE(idEtp, idEtp_inter)'))
                ->orderBy('etp_name', 'asc')
                ->get();
            ...
            $periodePrev3 = DB::table('v_union_projets')
                ...
                ->where('module_name', '!=', 'Default module')
                ->where('p_id_periode', "prev_3_month")
                ->whereIn('formateurs.idFormateur', $idFormateurs)
                ->groupBy('p_id_periode')
                ->first();
            // dd($periodePrev3);
    ...
    {
        $query = DB::table('projets')->where('idProjet', $id);
    ...
    public function linkInvitation(Request $req)
    {
        $query = DB::table('projets')->where('idProjet', $req->idProjet);
