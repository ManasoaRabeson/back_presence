...
class AbnCfpController extends Controller
{
    ...
    {
        $plans = Plan::where('user_type', 'centre de formation')->with('features')->get();
        return view('CFP.abonnements.index', compact('plans'));
    }
    ...
    public function recapAbn($id)
    {
        ...
        $plan = Plan::with('features')->findOrFail($id);
        return view('CFP.abonnements.recapPage', compact('plan', 'monthlyPrice', 'annualSavings', 'annualPrice', 'id_order'))->with(['pay' => $payment_iframe_html]);
    }
    ...
    public function subscribe(Request $request, $planId)
    {
        ...
        if ($existingSubscription) {
            if ($existingSubscription->plan_id == 1 || $existingSubscription->plan_id == 5) {
                ...
                $newStartDate = Carbon::parse($existingSubscription->ends_at);
        ...
        return redirect()->route('cfp.abonnement.forfait', $planId)->with('success', 'Vous avez souscrit au plan avec succès.');
    }
    ...
    public function showSubscriptions()
    {
        ...
        return view('CFP.abonnements.forfait', compact('payments', 'subscriptions', 'mysubscriptions'));
    }
}
